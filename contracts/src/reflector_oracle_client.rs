...
impl ReflectorOracleClient {
    ...
    pub fn fetch_latest_price(env: Env, asset: String, exchange: String) -> Result<PriceData, OracleError> {
        ...
        // Create a deterministic but varied price based on the inputs
        // We'll work directly with the String objects instead of converting to &str
        let price = simulate_price_from_strings(&asset, &exchange, env.ledger().timestamp());
    ...
    pub fn get_twap(env: Env, asset: String, period: u64) -> Result<i128, OracleError> {
        ...
        let twap_value = simulate_price_from_strings(&asset, &String::from_str(&env, "TWAP"), env.ledger().timestamp());
        let twap_value = (twap_value * (10000 - (period % 100) as i128)) / 10000; // Small variation based on period
...
// We'll work with String references directly
fn simulate_price_from_strings(asset: &String, exchange: &String, timestamp: u64) -> i128 {
    // Create a deterministic but varied price
    // We'll use a simple hash of the string contents
    let asset_bytes = asset.to_bytes();
    let exchange_bytes = exchange.to_bytes();
    
    let asset_hash = asset_bytes.iter().fold(0u64, |acc, b| acc.wrapping_add(b as u64));
    let exchange_hash = exchange_bytes.iter().fold(0u64, |acc, b| acc.wrapping_add(b as u64));
